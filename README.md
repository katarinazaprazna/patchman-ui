...
For running the integration tests, you will need to point Playwright to the stage directly. Follow these steps to
prepare your environment for running the tests:

1. Set `PROXY` and `BASE_URL` - see `playwright_example.env` for reference
2. Set the `INTEGRATION` flag to true
3. _This step is required to run integration tests using
   containers_. Set the `DOCKER_SOCKET` option in your .env file as follows:
   ```
   DOCKER_SOCKET="/tmp/podman.sock"
   ```
4. For Podman to serve the API for client testing, run this command:
    ```bash
    podman system service -t 0 unix:///tmp/podman.sock
    ```
5. Run the tests with `npx playwright test`

If you're using Docker instead of Podman, you can skip steps 3 and 4 and run the tests directly with
`npx playwright test`.
